...
const listOf = (inner: string, wrap: (s:string)=>string) =>
  (t: string) => { const m = t.match(/^List<(.+)>$/); return m ? wrap(m[1]) : null; };

function makeMapper(map: TMap, listFn: (inner: string) => string) {
  return function m(t: string): string {
    const li = listOf(listFn, m)(t);
    if (li) return li;
    return map[t] ?? t;
  };
}
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function typeMapper(lang: OopLang): (t: string) => string {
  switch (lang) {
